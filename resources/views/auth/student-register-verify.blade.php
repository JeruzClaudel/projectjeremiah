...
<div class="student-auth-card">
    ...
    <div class="student-auth-header">
        ...
        <p>Enter the 6-digit code sent to<br><strong style="color:#0a1931;">{{ $email }}</strong></p>
    </div>
    ...
    <form method="POST" action="{{ route('student.register.verify.post') }}" id="otp-form">
        ...
        {{-- Hidden OTP field filled by JS --}}
        <input type="hidden" name="otp" id="otp-hidden">

        <div class="otp-input-row" id="otp-boxes">
            @for($i = 0; $i < 6; $i++)
                <input type="text" maxlength="1" inputmode="numeric" pattern="[0-9]"
                       class="otp-digit" data-index="{{ $i }}"
                       autocomplete="off">
            @endfor
        </div>

        <button type="submit" class="btn-student-submit" id="verify-btn" disabled
    ...
    <div style="text-align:center;margin-top:20px;">
        <p style="font-size:.8rem;color:#6b7280;margin-bottom:10px;">
            Didn't receive the code? Code expires in <strong>5 minutes</strong>.
        </p>
        <form method="POST" action="{{ route('student.register.resend') }}" style="display:inline;">
            @csrf
            <button type="submit"
