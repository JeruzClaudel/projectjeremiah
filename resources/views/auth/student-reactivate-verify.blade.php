...
@section('title', 'Enter Reactivation Code — Project Jeremiah')

<div class="guest-card-header">
    <div class="guest-icon">🔑</div>
    <h1>Enter your code</h1>
    <p>6-digit code sent to<br><strong style="color:var(--navy);">{{ $email }}</strong></p>
</div>
...
<form method="POST" action="{{ route('student.reactivate.verify.post') }}" id="react-form">
    @csrf
    ...
    <div class="otp-row">
        @for($i = 0; $i < 6; $i++)
            <input type="text" maxlength="1" inputmode="numeric" pattern="[0-9]"
                   class="otp-digit react-digit" data-index="{{ $i }}" autocomplete="off">
        @endfor
    </div>

    <div class="form-actions" style="border-top:1px solid var(--sky);padding-top:18px;margin-top:4px;">
        <a href="{{ route('student.reactivate.request') }}" class="btn btn-secondary btn-sm">Back</a>
        <button type="submit" id="react-btn" class="btn btn-primary"
                disabled style="opacity:.5;cursor:not-allowed;">
            Reactivate account <span>↗</span>
        </button>
    </div>
</form>

<p style="text-align:center;color:var(--muted);font-size:.78rem;margin-top:16px;">
    Code expires in <strong>5 minutes</strong>. Max 3 attempts.<br>
    <form method="POST" action="{{ route('student.reactivate.resend') }}" style="display:inline;margin:0;">
        @csrf
        <input type="hidden" name="email" value="{{ $email }}">
        <button type="submit"
